      ...
      <div className="grid grid-cols-7 w-full">
        {Days.map((item: string) => {
          return (
            <div
              key={item}
              className="flex justify-center border p-3 bg-slate-100"
            >
              {item}
            </div>
          );
        ...
        {Dates.map((item: any, index: number) => {
          return (
            <div
              key={index}
              className={`${
                format(SelectedDate) == format(item) && "bg-blue-100"
              } ${
                format(Today) == format(item) && "bg-blue-200"
              } flex  justify-center border p-4 hover:bg-slate-100`}
              onClick={() => Event(item)}
              onDrop={(e) => HandleDragDrop(e, item)}
              onDragOver={HandleDragOver}
            >
              {item ? item?.getDate() : null}
              {events[format(item)]?.length > 0 && (
                <span className="p-1 bg-blue-800 rounded-full w-fit h-fit"></span>
              )}
            </div>
          );
        })}
